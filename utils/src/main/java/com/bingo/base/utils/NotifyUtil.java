...
public class NotifyUtil {
    ...
    public void initNotificationProgrss(Context context, Class<?> target) {
        ...
        PendingIntent pendingIntent = PendingIntent.getBroadcast(mContext,
                0, intent, 0);
        builder.setSmallIcon(R.drawable.lib_pub_icon_notify)// 设置通知小图标,在下拉之前显示的图标
                .setWhen(System.currentTimeMillis())
                .setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(), R.drawable.lib_pub_icon_notify))// 落下后显示的图标
    ...
    public void initRemoteViewsNotification(Context context, Class<?> target) {
        mContext = context;
        builder = new Notification.Builder(context);
        builder.setSmallIcon(R.drawable.lib_pub_icon_notify)
                .setWhen(System.currentTimeMillis())
                .setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(), R.drawable.lib_pub_icon_notify))// 落下后显示的图标
                //.setOngoing(true)// 不能被用户x掉，会一直显示，如音乐播放等
                .setAutoCancel(true)// 自动取消
